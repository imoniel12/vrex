...
class SchoolFormAdmin(admin.ModelAdmin):
    ...
    list_display = (
        ...
        'custom_date_released',
        'custom_status',
        'get_requested_by_username'  # Display the username of the requester
    )
    search_fields = ('get_unique_id', 'status', 'requested_by__username')  # Allow searching by requester's username
    list_filter = ('user_type', 'status')  # Add 'status' to list_filter if needed
    ...
    def get_requested_by_username(self, obj):
        return obj.requested_by.username if obj.requested_by else "N/A"

    get_requested_by_username.short_description = 'Requested By'
...
class ReportAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('infographics',)

    def infographics(self, obj):
        # Render the chart template with the required data
        total_requests = SchoolForm.total_request_count()
        ongoing_requests = SchoolForm.ongoing_request_count()
        monthly_requests = SchoolForm.monthly_document_requests()
        recent_actions = SchoolForm.recent_actions()
        ...
        chart_html = render_to_string('vrexApp/admin/app_list.html', chart_data)
        return chart_html

    infographics.short_description = 'Infographics'

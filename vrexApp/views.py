...
def formrequest(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Create a new instance of SchoolForm and populate it with form data
            school_form_instance = SchoolForm(
                first_name=form.cleaned_data['first_name'],
                middle_name=form.cleaned_data['middle_name'],
                last_name=form.cleaned_data['last_name'],
                user_type=form.cleaned_data['user_type'],
                purpose=form.cleaned_data['purpose'],
                requestform=form.cleaned_data['requestform'],
                request_date=form.cleaned_data['request_date'],
                specialinstructions=form.cleaned_data['specialinstructions'],
            )
            # Set the requested_by field to the currently logged-in user
            school_form_instance.requested_by = request.user
            school_form_instance.save()
...
def checkstatus(request):
    ...
    requested_forms = SchoolForm.objects.filter(user_type=request.user.user_type, requested_by=request.user)
...
def calendar(request):
    ...
    return render(request, 'vrexApp/calendar.html', {'today': current_date})
